    ...
    def get_average_temperature():
        ...
        try:
            ...
            response = requests.get(
                f"{API_URL}/statistics/descriptive?phenomenon=Temperatur&from-date="
                f"{start_time}&to-date={end_time}&window=3600000&"
                f"operation=arithmeticMean&bbox=-180,-90,180,90&format=json", timeout=60
                )
            ...
            if response.headers.get('content-type') == 'application/json':
                ...
                temperatures = []
                for record in data:
                    temperatures.extend(float(value) for key, value in record.items() if key != 'sensorId')                
                if temperatures:
            #        print(temperatures) 
                    average_temperature = sum(temperatures) / len(temperatures)
                    return jsonify({'average_temperature': f"{average_temperature:.2f} °C"})
                else:
...
if __name__ == "__main__":
    ...
    launch_app()
    app.run(debug=True, host='0.0.0.0', port=5000) # Adjust the host and port if needed
# def version():
#     version = "v0.0.1" 
#     return version